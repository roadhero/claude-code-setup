...
Per-repo overrides (drop into <repo>/.claude/agents/ — override the 15 globals by canonical name):
   ...
   Compute repo → agents-compute/    (13; 5 overrides of generic agents (the four-hat chain architect/senior-swe/code-reviewer/qa, plus performance-engineer) + 8 compute-only specialists — the 7 byte-identical global copies and the release-engineer near-dupe were dropped so those names fall through to ~/.claude/agents/)
