...
6. **Cited paths ↔ files:** every path a doc, agent, skill, or hook cites resolves. Cheap first pass: `grep -ohE '[A-Za-z0-9_.-]+(/[A-Za-z0-9_.-]+)+\.(sh|md|yml|json|tmpl)' <files>` then `test -e` on each; only tokens with a `/` are worth checking, and the survivors still need a human eye (scaffolder-generated files and diff headers are not dangling). Every intended duplicate (a template and its scaffolder copy) is byte-identical or has a `diff -q` in the quality gate. A dangling reference or a silently drifted copy is drift, not a cosmetic nit.
